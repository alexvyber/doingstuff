...
config CLI-app that work like this: you call it with name of the program that you want to config and
it opens the config file fo that programm in default editor.
...
App that gives tips and tricks about linux and apps based on apps that installed on the system. This
thing analyses the system and generates personalized tips.
...
Online Timer with labels that can write some stuff to some API after it's done. Timer has to
remember its users! And continue counting even if connection is lost!
...
use draginfly as in-memory db
use pocketbase as api for all the rest
