...
Make sure that all paths are right. The example above assumes that all files are located in the same
directory.
...
You can only import a type from a different file, if it has been exporting using named or default
export.

You can have **as many named exports as you want** per file, but **only one default export** per
file.
