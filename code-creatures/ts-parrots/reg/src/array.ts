...
;(Object.entries(Obj) as Array<[key: keyof typeof Obj, value: (typeof Obj)[keyof typeof Obj]]>).map(
  ([key, value]) => {},
)
...
const filteredFriendsAsserted = friends.filter((item): item is NonNullable<(typeof friends)[number]> => Boolean(item))
