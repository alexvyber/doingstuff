// sourced from https://www.cult-ui.com/docs/components/dock
...
function Dock({ className, children }: DockProps) {
  ...
  const mouseX = useMotionValue(Number.POSITIVE_INFINITY) // Motion value to track the mouse's X position relative to the viewport. Initialized to Infinity to denote no tracking initially.
  ...
  return (
    ...
    >
      <motion.div
        ...
        onMouseLeave={() => {
          mouseX.set(Number.POSITIVE_INFINITY) // Reset mouseX motion value
...
function DockCardInner({ src, id, children }: DockCardInnerProps) {
  ...
  return (
    <span className="relative z-0 flex h-full w-full items-center justify-center overflow-hidden rounded-md">
      ...
      <motion.img
        className="absolute z-10 translate-y-2.5 scale-125 transform opacity-25 blur-md filter"
        src={src}
        alt=""
      />
      ...
      <AnimatePresence mode="popLayout">
        {animatingIndexes.includes(Number.parseInt(id)) ? null : (
          <motion.img
            layoutId={id} // Unique identifier for layout animations
            className="relative z-0 h-2/3 w-2/3 rounded-full"
            src={src}
            alt=""
          />
        )}
...
function DockCard({ children, id }: DockCardProps) {
  ...
  const handleClick = () => {
    ...
    if (isAnimating.current) {
      ...
      dock.setAnimatingIndexes(dock.animatingIndexes.filter((index) => index !== Number.parseInt(id)))
  ...
  return (
    ...
    >
      ...
      >
        {children} {/* Render the children of the DockCard inside the motion button */}
      </motion.button>
