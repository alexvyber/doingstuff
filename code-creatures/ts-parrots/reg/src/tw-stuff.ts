...
export const mergeVariants = <T, U>(baseVariant: Config<T>, newVariant: Config<U>) => {
...
function getCompoundVariants<T extends readonly any[], U extends readonly any[]>(
  ...
  for (const [key, { className, ...rest }] of (arr as Array<{ className: string; [key: string]: string }>).entries()) {
...
function getVariants<T extends ConfigSchema, U extends ConfigSchema>(baseVariants: T, newVariants: U): T & U {
  // if (!newVariants) return baseVariants
  // if (!baseVariants) return newVariants
  ...
  ;(
    ...
    (Object.keys(value) as Array<keyof typeof value>).map((key: keyof typeof value) => {
      if (!obj[variant]) Object.assign(obj, { [variant]: newVariants[variant] })
